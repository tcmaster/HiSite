...
public class XListView extends ListView implements OnScrollListener {
	...
	/**
	 * set last refresh time 璁剧疆鏄剧ず鐨勫埛鏂版椂闂?
	 * 
	 * @param isNowTime
	 *            true鍒锋柊鏂扮殑鏃堕棿锛宖lase鍒锋柊褰撳墠鏃堕棿
	 */
	public void setRefreshTime(boolean isNowTime) {
		if (isNowTime) {
			date = new Date();// 鏈?杩戞洿鏂版椂闂?
		}
		String dateStr = StringUtils.getDateString(date);// 将日期转换为相关字符串
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (getFirstVisiblePosition() == 0 && (mHeaderView.getVisiableHeight() > 0 || deltaY > 0)) {
				// the first item is showing, header has shown or pull down.涓嬫媺鍒锋柊
