...
public class LiveSubjectNativeController {
	...
	public void InsertData(LiveSubjectModel liveSubjectModel, String eventId) {
		...
		// 存到数据库中
		DBUtil.saveOrUpdate(subjectEntity, SubjectEntity.class, "content", "uid", "rid");
		DBUtil.saveOrUpdateAll(listPhotoEntitiys, PhotoEntitiy.class, "url", "size", "rid", "uid");
	...
	// SubjectEntity subjectEntity = DBUtil.getDataFirst(SubjectEntity.class, "rid = " + eventId +
	// " and uid = " + userId);
	...
	// // 图片
	// List<PhotoEntitiy> photoEntitiy = DBUtil.getData(PhotoEntitiy.class, "rid = " + subjectEntity +
	// " and uid = " + userId);
	...
	// }
	// // 评论
	// List<CommentEntity> commentEntitiys = DBUtil.getData(CommentEntity.class, "rid = " +
	// subjectEntity.getId() + " and uid = " + userId);
